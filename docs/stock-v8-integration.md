...
- classic source compilation and execution plus browser-resolved ES-module
  compile/instantiate/evaluate with URL, line, column, and stack diagnostics;
...
20. an HTTP-loaded, Vite-shaped module graph boots production React through the
    navigation scheduler, observes `loading`, `interactive`,
    `DOMContentLoaded`, `complete`, and `load`, preserves one module evaluation
    for duplicate root tags, updates through delegated input, then unmounts and
    tears down with zero native ownership.
...
The initial document lifecycle and module-loading contract is recorded in
[`docs/production-page-boot.md`](production-page-boot.md).
