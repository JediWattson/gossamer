...
func Render(document *dom.Node, viewport Viewport) (*Frame, error) {
	...
	defer fonts.Close()
	return renderWithFonts(document, viewport, fonts)
}
...
func RenderPNG(writer io.Writer, document *dom.Node, viewport Viewport) error {
	...
	frame, err := renderWithFonts(document, viewport, fonts)
	if err != nil {
		return err
	}
...
func renderWithFonts(document *dom.Node, viewport Viewport, fonts *fontBook) (*Frame, error) {
	...
	styledRoot := buildStyleTree(document, viewport)
	rootBox, styles, err := layoutDocument(styledRoot, viewport, fonts)
	if err != nil {
	...
	for _, fragment := range box.Text {
		...
		if fragmentStyle, ok := styles[fragment.Node]; ok && fragmentStyle.underline {
			list.Commands = append(list.Commands, Command{
				...
				Rect: Rect{
					X:      fragment.X,
					Y:      fragment.BaselineY + math.Max(1, fragment.FontSize/16),
					Width:  fragment.Width,
					Height: math.Max(1, fragment.FontSize/16),
				},
				Color: fragment.Color,
			})
		}
