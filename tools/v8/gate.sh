...
"${GOSSAMER_ROOT}/tools/v8/test.sh" \
  -run 'TestStockV8(EvaluationMicrotasksProfilingAndTeardown|FocusedWebPlatformSubsets|FormSubmissionNavigatesAndTearsDownOldRealm|ReactDOMCompatibilityGate|CSSOMViewRootScrollAndReactMeasurement|AnimationFramesPerformanceClockAndViewportResize|ResizeAndIntersectionObserversRetainTargetsAndTeardown|ReactPositionedLayoutStackingAndFixedGeometry|ReactFlexLayoutGeometryAndDelegatedHitTesting|InteractiveWindowRoutesNativeInputAndTeardown|ReplacementSubmissionLeakAndPerformanceGate)' \
  -count=1 \
  -timeout=60s
